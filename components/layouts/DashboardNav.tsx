...
export default function DashboardNav({ user, warehouses, activeWarehouseId, isAdmin }: DashboardNavProps) {
  ...
  // Build navigation array with conditional admin link
  const navigationItems = [
    ...
    { name: 'Job Works', href: '/dashboard/job-works', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    { name: 'Account Status', href: '/dashboard/account-status', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  // Add admin link for superadmins only
  if (user?.is_superadmin && !user?.is_demo) {
    navigationItems.push({
      name: 'Upgrade Requests',
      href: '/dashboard/admin/invite-requests',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
        </svg>
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          <Link href="/dashboard" className="flex items-center gap-2">
            <Image src="/logo.jpeg" alt="Bale Logo" width={32} height={32} className="rounded-lg" />
            <span className="text-xl font-bold text-brand-blue">Bale</span>
          </Link>
          <button
            ...
            <div className="px-4 py-3 space-y-1">
              ...
              <div className="pt-3 mt-3 border-t border-gray-200">
                ...
                <button
                  ...
                  className="w-full mt-2 flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-100 transition-colors"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
